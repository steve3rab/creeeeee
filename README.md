...
holds real unit tests written with
[doctest](https://github.com/doctest/doctest) (`tests/doctest.h`,
vendored as a single header, MIT license — not a hand-rolled
`main()`/`assert()` script: each `TEST_CASE` is its own independently
registered, independently reported test, with proper
pass/fail/file:line output per assertion; `DOCTEST_CONFIG_IMPLEMENT_WITH_MAIN`
has doctest itself generate `main()`):

- `test_scope_guard.cpp` — `creo::ScopeGuard`/`Defer()`, 7 cases. Zero
  ProTOOLKIT/Win32 dependency, so it builds and runs on any platform:
  normal/exception scope exit, `Dismiss()` (with and without a following
...
- `test_property_utils.cpp` — `PropertyUtils` (`windows/PropertyUtils.hpp`),
  ...
  non-ASCII), empty-string handling, throwing on malformed UTF-8, and
...
Each test executable also runs standalone with doctest's own CLI, e.g.
